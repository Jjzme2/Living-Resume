    ...
    <form @submit.prevent="handleSubmit" class="post-form">
      ...
      <div class="action-row">
        <div class="action-left">
          <span v-if="githubAvailable" class="github-badge">
        ...
        <div class="action-right">
          ...
          <button v-if="githubAvailable" type="submit" class="btn btn-primary" :disabled="!canSubmit || saving">
...
const { data: githubCheck } = await useAsyncData('github-check', () =>
  $fetch<{ available: boolean }>('/api/admin/blog/github-check').catch(() => ({ available: false }))
)
const githubAvailable = computed(() => githubCheck.value?.available ?? false)
...
async function handleSubmit() {
  ...
  try {
    const filePath = `content/blog/${form.category}/${form.slug}.md`
    await $fetch('/api/admin/blog/create', { method: 'POST', body: { filePath, content: generatedMarkdown.value } })
    success.value = `Pushed: ${filePath}. Vercel will redeploy automatically.`
  } catch (e: unknown) {
    error.value = (e as { data?: { message?: string } })?.data?.message ?? 'Failed to push to GitHub.'
  } finally {
    saving.value = false
  }
}
...
.github-badge { display: inline-flex; align-items: center; gap: var(--sp-2); font-size: 0.72rem; font-family: var(--font-mono); color: var(--text-3); background: var(--bg-elevated); border: 1px solid var(--border-xs); border-radius: 99px; padding: 0.3em 0.8em; }
